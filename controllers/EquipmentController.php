<?php
   ...
   class EquipmentController extends DatabaseController
   {
        public function getShopRecords()
        {
            global $db;
            $date = new DateTIme('-30');
            $date_format = $date->format('Y/m/d H:i:s');
            $query = "select * from shop where timestamp >= '{$date_format}'";
            $shops = mysqli_query($db->connection,$query);

            return $shops;
        }

        public function createEquipmentRecords()
        {
        ...
        public function updateEquipmentRecords()
        {
            global $db;
        ...
        public function saveUpdateEquipmentRecords($id)
        {
            if(isset($_POST['btn_save']))
            {
                global $db;
